...
od
    ...
    \{updateRpost\} =  \{f(i + 1) = r $\land$ f(i) = r - x $\land$ y  $\leq$ n $\land$ n $\geq$ 1\}\\
    \{updateXpost\} =  \{f(i + 1) = r $\land$ f(i) = x $\land$  i $\leq$ n $\land$ n $\geq$ 1\}\\    
    Below this proof i will also prove my proof obligations. The invariant is very similair to the one given in the lecture for the power program example. The overall proof is a mix between the lecture example for factorial and power function.\\
    \begin{enumerate}[1. ]
    ...
    \begin{list}
        \item {Proof for Line 11 - (invariant $\land$ i $<$ n) $\to$ (updateRpre)} 
            ...
            (r = f(i) $\land$ i $<$ n $\land$ x = f(i -1) $\land$ i $\leq$ n) $\land$ n $\geq$ 1) $\to$ (f(i + 1) = r + x $\land$ f(i) = x $\land$ i $\leq$ n $\land$ n $\geq$ 1)\\
            ...
            if r = f(i) and x = f(i - 1) then that means f(i + 1) = r + x for all Fibonacci numbers greater than or equal to 1.\\
            ...
            f(i) = f(i - 1) + f(i - 2) which is our definition of the Fibonacci sequence.\\\\ Please note that f(i) = f(i - 1) + f(i - 2) is also the same as f(i + 2) = f(i + 1) + f(i) since we have f(0) and f(1) defined we never run into negative Fibonacci numbers and we can substitute i = i - 2 since i $\geq$ 1.
    \end{list}
    ...
    \begin{list}
        ...
        So our proof becomes:\\
        show that i = n by the end of the loop to get f(n) by doing a substitution. we have i $\geq$ n and our invariant states that r = f(i) and x = f(i - 1) and i $\leq$ n. by combining the fact that i $\leq$ n and that i $\geq$ n in our first part of the implication, using a variation of squeezes theorem, if n $\geq$ i $\leq$ n. in a less formal way to put it, i is less than or equal to a number and greater than or equal to the same number, then i is that number. This gives us i = n, and r = f(n), allowing for post condition weakening. 
    \end{list}
    Therefore our proof is complete, since we have come with {T} \{TRUE $\land$ n $\geq$ 1\} Fib \{r = f(n)\}, then assuming our program Fib terminates we get the following deduction:\\ Since this was deduced with no presumptions that means everything deduces that if n $\geq$ 1 then if we run our Fib program and it terminates, then r = f(n).\\
