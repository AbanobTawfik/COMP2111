...
\begin{enumerate}[(a)]
  ...
  $\neg (F) = T$
  \begin{enumerate}[(i)]
  \item Show that $\psi_1 \wedge \psi_2 \wedge \psi_3$ is satisfiable; so the requirements can all be met. Note that it is sufficient to give a satisfying truth assignment, you do not have to list all possible combinations. \moveToRight{4 marks}\\
  Solution: \\
  ...
  \hspace*{11pt} $= T \wedge T$ \moveToRight{using rules of propositional logic above}\\
  \hspace*{11pt} $= T$ \moveToRight{using rules of propositional logic above}\\
  we have shown with these assignments that $\psi_1$ = $\psi_2$ = $\psi_3$ = T\\
  ...
  hence $\psi_1 \wedge \psi_2 \wedge \psi_3$ is satisfiable as it equates to true with the following propositional variables (A$_L$ = T,
  A$_H$ = F,
  B$_L$ = F,
  B$_H$ = T,
  C$_L$ = T,
...
\begin{centering}\hspace*{1pt}\includegraphics[width=300px, height = 300px]{3b.png}\captionof{figure}{Verification tool returning that my proof is correct for part (a)}\end{centering}
\end{flushleft}
\fixindent{}
